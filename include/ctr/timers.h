...
/*! \file timers.h
    \brief Contains defines, macros and functions for ARM9 timer operation.
    It also contains a simplified API for timer use and some cpu timing functions.

	The timers are fed with a 67.027964 MHz source on the ARM9.
*/


#ifndef CTR_TIMERS_INCLUDE
#define CTR_TIMERS_INCLUDE
...
/*!	<b>Example Usage:</b> %TIMER_CR(x) = %TIMER_ENABLE | %ClockDivider_64;
	...
	\see ClockDivider
*/
#define TIMER_CR(n) (*(vu16*)(0x10003002+((n)<<2)))

//!	Same as %TIMER_CR(0).
#define TIMER0_CR   (*(vu16*)0x10003002)
//!	Same as %TIMER_CR(1).
#define TIMER1_CR   (*(vu16*)0x10003006)
//!	Same as %TIMER_CR(2).
#define TIMER2_CR   (*(vu16*)0x1000300A)
//!	Same as %TIMER_CR(3).
#define TIMER3_CR   (*(vu16*)0x1000300E)
...
*/
#define TIMER_DATA(n)  (*(vu16*)(0x10003000+((n)<<2)))

//!	Same as %TIMER_DATA(0).
#define TIMER0_DATA    (*(vu16*)0x10003000)
//!	Same as %TIMER_DATA(1).
#define TIMER1_DATA    (*(vu16*)0x10003004)
//!	Same as %TIMER_DATA(2).
#define TIMER2_DATA    (*(vu16*)0x10003008)
//!	Same as %TIMER_DATA(3).
#define TIMER3_DATA    (*(vu16*)0x1000300C)



//! the speed in which the timer ticks in hertz.
#define BUS_CLOCK (67027964)
...
typedef enum {
	ClockDivider_1 = 0,		//!< divides the timer clock by 1 (~67027.964 kHz)
	ClockDivider_64 = 1,	//!< divides the timer clock by 64 (~1047.314 kHz)
	ClockDivider_256 = 2,	//!< divides the timer clock by 256 (~261.828 kHz)
	ClockDivider_1024 = 3	//!< divides the timer clock by 1024 (~65.4568 kHz)
}ClockDivider;


//use the ClockDivider enum.
//	Causes the timer to count at 67.028 Mhz.
#define TIMER_DIV_1     (0)
//	Causes the timer to count at (67.028 / 64) Mhz.
#define TIMER_DIV_64    (1)
//	Causes the timer to count at (67.028 / 256) Mhz.
#define TIMER_DIV_256   (2)
//	Causes the timer to count at (67.028 / 1024) Mhz.
...
/*!	\brief start a hardware timer.
	...
	\param divider 	The timer channel clock divider (clock will tick at 67.027964 Mhz / divider)
